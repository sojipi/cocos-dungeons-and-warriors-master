...
class EventManager {
    ...
    on<T>(key: string, func: (params: T) => void, ctx: any) {
        console.log('[EventManager] Registering event listener for:', key);
    ...
    emit(key: string, ...params: any[]) {
        console.log('[EventManager] Emitting event:', key, 'with params:', params);
        if (!this.event_map.has(key)) {
            console.log('[EventManager] No listeners for event:', key);
            return;
        }
    ...
    off(key: string, ctx: any) {
        console.log('[EventManager] Removing event listener for:', key);
